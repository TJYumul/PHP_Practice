...
if ($keyword !== '') {
  foreach ($listings as $listing) {
    if (str_contains(strtolower($listing['location']), $keyword)) {
      $results[] = $listing;
    }
  }
}
    ...
    <?php foreach ($results as $result): ?>
      <div class="bg-white rounded-lg shadow-md mb-4">
        <div class="p-4">
          ...
          <p class="text-gray-700 text-lg mt-2"><?= $result['description'] ?></p>
          <ul class="mt-4">
            <li class="mb-2"><strong>Salary:</strong> <?= $result['salary'] ?></li>
            <li class="mb-2"><strong>Location:</strong> <?= $result['location'] ?></li>
            <li class="mb-2"><strong>Tags:</strong> <?= implode(', ', $result['tags']) ?></li>
          </ul>
        </div>
      </div>
    <?php endforeach; ?>
